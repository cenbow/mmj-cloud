package com.qianmi.open.api.qmcs.schedule;
...
public class Scheduler<T> {
	...
	protected Log logger = LogFactory.getLog(Scheduler.class);
	private Object lock;
	...
	public void schedule(T t, Runnable task) throws ChannelException {
		...
		if (this.haveReachMaxPendingCount(t, queue, task))
			throw new ChannelException(String.format(Text.SCHEDULE_GOT_MAX, this.max));

		try {
			queue.add(task);
		} catch (Exception e) {
			throw new ChannelException(Text.SCHEDULE_TASK_REFUSED, e);
		}
	...
	protected final void dispatch() {
		...
		if (this.logger.isDebugEnabled() && c > 0)
			this.logger.debug(String.format(Text.SCHEDULE_TASK_DISPATCHED, c));
	}
