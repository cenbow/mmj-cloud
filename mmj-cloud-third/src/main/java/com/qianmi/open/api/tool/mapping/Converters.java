	...
	public static <T> T convert(Class<T> clazz, Reader reader) throws ApiException {
		...
		try {
			...
			for (PropertyDescriptor pd : pds) {
				...
				Field field;
				if (baseFields.contains(itemName) && com.qianmi.open.api.QianmiResponse.class.isAssignableFrom(clazz)) {
					field = getField(com.qianmi.open.api.QianmiResponse.class, pd);
				} else {
